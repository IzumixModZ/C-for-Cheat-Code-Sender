...
namespace CHEAT_SAMPLE_FOR_CSharp
{
    ...
    public class TCPGecko
    {
        ...
        private const Byte cmd_pokeu = 0x0A;
        private const Byte cmd_poke_registers = 0xA7;
        ...
        public void poke(UInt32 address, UInt64 value)
        {
            //Lower address
            address &= 0xFFFFFFFC;
            ...
            //Send poke
            if (this.RawCommand(cmd_poke_registers) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
        ...
        public void byteary_poke(UInt32 b_address, byte[] b_value)
        {
            Console.WriteLine(string.Join(", ", b_value));
            UInt32 value;

            value = (UInt32)b_value[0] << 24 |
                    (UInt32)b_value[1] << 16 |
                    (UInt32)b_value[2] << 8 |
                    (UInt32)b_value[3];

            //Lower address
            b_address &= 0xFFFFFFFC;

            //value = send [address in big endian] [value in big endian]
            UInt64 PokeVal = (((UInt64)b_address) << 32) | ((UInt64)value);
        ...
        public void byte_poke(byte[] b_address, byte[] b_value)
        {
            Console.WriteLine(string.Join(", ", b_address) + string.Join(", ", b_value));
            UInt32 address, value;
            address = (UInt32)
                (UInt32)b_address[0] << 24 |
                (UInt32)b_address[1] << 16 |
                (UInt32)b_address[2] << 8 |
                (UInt32)b_address[3];
        ...
        internal void poke08(int v1, byte[] v2)
        {
